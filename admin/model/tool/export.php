...
class ModelToolExport extends Model {
	...
	function storeProductsIntoDatabase( &$database, &$products ) 
	{
		...
		foreach ($products as $product) {
			...
			$sql2 .= "($productId,$languageId,'$productName','$productDescription','$meta_description','$meta_keywords','$meta_title');";
			$database->query($sql);
			$database->query($sql2);
	...
	function storeCategoriesIntoDatabase( &$database, &$categories ) 
	{
		...
		foreach ($categories as $category) {
			...
			$sql2 .= " $status);";
			$database->query( $sql2 );
			$sql3 = "INSERT INTO `".DB_PREFIX."category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, `meta_title`) VALUES ";
			$sql3 .= "( $categoryId, $languageId, '$name', '$description', '$meta_description', '$meta_keywords', '$meta_title' );";
			$database->query( $sql3 );
			if ($keyword) {
	...
	function validateCategories( &$reader )
	{
		...
		( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "language_id", "seo_keyword", "description", "meta_description", "meta_keywords", "meta_title", "store_ids", "layout", "status\nenabled" );
		$data =& $reader->getSheet(0);
	...
	function validateProducts( &$reader )
	{
		...
		( "product_id", "name", "categories", "sku", "upc", "location", "quantity", "model", "manufacturer", "image_name", "requires\nshipping", "price", "points", "date_added", "date_modified", "date_available", "weight", "unit", "length", "width", "height", "length\nunit", "status\nenabled", "tax_class_id", "viewed", "language_id", "seo_keyword", "description", "meta_description", "meta_keywords", "meta_title", "additional image names", "stock_status_id", "store_ids", "layout", "related_ids", "tags", "sort_order", "subtract", "minimum" );
		$data =& $reader->getSheet(1);
	...
	function populateCategoriesWorksheet( &$worksheet, &$database, $languageId, &$boxFormat, &$textFormat )
	{
		...
		$worksheet->setColumn($j,$j++,max(strlen('meta_title'),32)+1);
		...
		$worksheet->writeString( $i, $j++, 'meta_title', $boxFormat );
	...
	function populateProductsWorksheet( &$worksheet, &$database, &$imageNames, $languageId, &$priceFormat, &$boxFormat, &$weightFormat, &$textFormat )
	{
		...
		$worksheet->setColumn($j,$j++,max(strlen('meta_title'),32)+1,$textFormat);
		...
		foreach ($result->rows as $row) {
			...
			$worksheet->write( $i, $j++, html_entity_decode($row['meta_title'],ENT_QUOTES,'UTF-8'), $textFormat );
			$names = "";
