	...
	public function sendVoucher($voucher_id) {
		...
		if ($voucher_info) {
			...
			if ($order_info) {
                ...
                if (!empty($voucher_theme_info['email_subject']))
                {
                    $subject      = $voucher_theme_info['email_subject'];
                    $greeting     = $voucher_theme_info['email_greeting'];
                    $from         = $voucher_theme_info['email_from'];
                    $message      = $voucher_theme_info['email_message'];
                    $redeem       = $voucher_theme_info['email_redeem'];
                    $footer       = $voucher_theme_info['email_footer'];
                } else {
				...
				$template->data['title'] = sprintf($subject, $voucher_info['from_name']);
				
				$template->data['text_greeting'] = sprintf($greeting, $this->currency->format($voucher_info['amount'], $order_info['currency_code'], $order_info['currency_value']));
				$template->data['text_from'] = sprintf($from, $voucher_info['from_name']);
				$template->data['text_message'] = $message;
				$template->data['text_redeem'] = sprintf($redeem, $voucher_info['code']);
				$template->data['text_footer'] = $footer;	
                                
                                $template->data['to_name'] = $voucher_info['to_name'];
                                $template->data['code'] = $voucher_info['code'];
				
				if (file_exists(DIR_IMAGE . $voucher_theme_info['image'])) {
				...
				if (file_exists(DIR_TEMPLATE . DIR_CATALOG_TEMPLATE . $this->config->get('config_template') . '/template/mail/voucher.tpl')) {
					$html = $template->fetch(DIR_CATALOG_TEMPLATE . $this->config->get('config_template') . '/template/mail/voucher.tpl');
				} else {
					$html = $template->fetch(DIR_CATALOG_TEMPLATE . 'default/template/mail/voucher.tpl');
				}
				...
				$mail->setSubject(html_entity_decode(sprintf($subject, $voucher_info['from_name']), ENT_QUOTES, 'UTF-8'));
				$mail->setHtml($html);
				$mail->send();
			...
			// If voucher does not belong to an order				
                ...
                if (!empty($voucher_theme_info['email_subject']))
                {
                    ...
                    $greeting     = $this->language->get('text_greeting');
                    $from         = $this->language->get('text_from');
                    $message      = $this->language->get('text_message');
                    $redeem       = $this->language->get('text_redeem');
