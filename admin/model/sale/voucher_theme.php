...
class ModelSaleVoucherTheme extends Model {
	public function addVoucherTheme($data) {
		...
		foreach ($data['voucher_theme_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "voucher_theme_description SET voucher_theme_id = '" . (int)$voucher_theme_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', email_subject = '" . $this->db->escape($value['email_subject']) . "', email_greeting = '" . $this->db->escape($value['email_greeting']) . "', email_from = '" . $this->db->escape($value['email_from']) . "', email_message = '" . $this->db->escape($value['email_message']) . "', email_redeem = '" . $this->db->escape($value['email_redeem']) . "', email_footer = '" . $this->db->escape($value['email_footer']) . "'");
		}
	...
	public function getVoucherThemeDescriptions($voucher_theme_id) {
		...
		foreach ($query->rows as $result) {
			$voucher_theme_data[$result['language_id']] = array('name' => $result['name'], 'email_subject' => $result['email_subject'], 'email_greeting' => $result['email_greeting'], 'email_from' => $result['email_from'], 'email_message' => $result['email_message'], 'email_redeem' => $result['email_redeem'], 'email_footer' => $result['email_footer']);
		}
