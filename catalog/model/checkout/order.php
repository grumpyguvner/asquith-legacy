...
class ModelCheckoutOrder extends Model {
    ...
    public function addOrder($data) {
        ...
        foreach ($data['totals'] as $total) {
            ...
            if (!empty($this->session->data['giftwrapping'])) {
                ...
                foreach ($this->session->data['giftwrapping'] as $giftwrap) {

                    if (!empty($giftwrap)) {
                        $this->db->query("INSERT INTO " . DB_PREFIX . "gifts_order SET gift_id = '" . (int) $giftwrap['giftwrap_id'] . "', order_id = '" . (int) $order_id . "', product_id = '" . (int) $giftwrap['product_id'] . "', price = '" . $giftwrap['price'] . "', name = '" . $giftwrap['giftwrap_name'] . "' ");
                    }
                }
            }
    ...
    public function getOrder($order_id) {
        ...
        if ($order_query->num_rows) {
            ...
            return array(
                ...
                'recommend_id' => $order_query->row['recommend_id'],
    ...
    public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
        ...
        if ($order_info && !$order_info['order_status_id']) {
            ...
            $subject = $this->config->get('config_email_order_subject');
            $greeting = $this->config->get('config_email_order_body');

            if (empty($subject)) 
            {
                $subject = sprintf($language->get('text_new_subject'), $order_info['store_name'], $order_id);
            } else {
                $subject = sprintf($subject, $order_id);
            }

            if (empty($greeting))
            {
                $greeting = sprintf($language->get('text_new_greeting'), $order_info['store_name']);
            }
            ...
            $template->data['text_greeting'] = html_entity_decode($greeting, ENT_QUOTES, 'UTF-8');
            ...
            // Text Mail
            $text = html_entity_decode($greeting, ENT_QUOTES, 'UTF-8') . "\n\n";
            ...
            // Recommend friend vouchers
             if ($order_info['recommend_id']) {
                 $this->load->model('account/recommend');
                 $this->model_account_recommend->sendNewOrderVoucher($order_info['recommend_id']);
             }
