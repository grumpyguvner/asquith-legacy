...
class ControllerNewsArticle extends Controller {
	public function index() {
		...
		$this->data['news_id'] = $this->request->get['news_id'];
			...
			if ($news_info) {
				$this->document->setTitle(!empty($news_info['meta_title']) ? $news_info['meta_title'] : $news_info['title']); 
	...
	public function writecomment() {
		...
		/* Added 25/7/12 TF : Turn Captcha on and off */
		if ($this->config->get('config_captcha_status') && (!isset($this->session->data['captcha']) || ($this->session->data['captcha'] != $this->request->post['captcha']))) {
